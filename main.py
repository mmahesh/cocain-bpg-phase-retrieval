...
Paper authors: Mahesh Chandra Mukkamala, Peter Ochs, Thomas Pock and Shoham Sabach.
...
parser.add_argument('--abs_fun_num', '--abs_fun_num', default=1,type=int,  dest='abs_fun_num')
...
for i in range(100):
	...
	global_L = global_L + 3*(np.linalg.norm(A)**2) + np.linalg.norm(A)*abs(temp_b)
...
def make_update(y,grad,uL_est):
	...
	coeff = [temp_p.T.dot(temp_p), 0,1, -1]
...
def find_gamma(A,b,U,prev_U,uL_est, lL_est):
	...
	while (kappa*breg(prev_U,U,breg_num=breg_num, A=A, b=b, lam=lam)<breg(U, y_U,breg_num=breg_num,  A=A, b=b, lam=lam)):
		gamma = gamma*0.9
		y_U = U+ gamma*(U-prev_U)
	return y_U, gamma


def do_lb_search(A,b, U, U1, lam, uL_est,lL_est):
...
def do_ub_search(A,b, y_U, uL_est):
	...
	# compute gradients
	grad_u = grad(A,b, y_U, lam, fun_num=fun_num)[0]
	...
	while((abs_func(A,b, x_U,y_U, lam, abs_fun_num = abs_fun_num, fun_num=fun_num)\
		-main_func(A,b, x_U, lam,  fun_num=fun_num)\
		+(uL_est*breg(x_U,y_U, breg_num=breg_num, A=A, b=b, lam=lam)))<-1e-7):
		uL_est = (2)*uL_est
...
def obtain_delta(A,b, y_U, uL_est):
	...
	if fun_num==1:
		...
		raise
	temp_delta = (abs_func(A,b, tx_U,y_U, lam, abs_fun_num = abs_fun_num, fun_num=fun_num)\
...
def line_search(y_U):
	...
	while(main_func(A,b, x_U, lam,  fun_num=fun_num) - main_func(A,b, y_U, lam,  fun_num=fun_num) -(eta*gm*delta) > 1e-7) and (delta >0):
		eta = eta*0.1
		x_U = y_U + eta*(tx_U - y_U)
...
if algo==1:
	...
	for i in range(max_iter):
		st_time = time.time()
		lL_est, y_U,gamma = do_lb_search(A,b, U, prev_U,  lam, uL_est,lL_est=lL_est_main)
		prev_U = U
		...
		lyapunov_vals = lyapunov_vals + [(1/uL_est)*temp+breg( U, prev_U, breg_num=breg_num, A=A, b=b, lam=lam)]
...
if algo==2:
	...
	time_vals = np.zeros(max_iter+1)
	time_vals[0] = 0
	#while(((1/uL_est)*grad(x,fun_num=fun_num) + grad(x,fun_num=fun_num))>1e-8):
	for i in range(max_iter):
		st_time = time.time()
	...
	filename = 'results/gd_bt_'+str(fun_num)+'_abs_fun_num_'+str(abs_fun_num)+'.txt'
...
if algo==3:
	...
	for i in range(max_iter):
		...
		func_vals = func_vals + [temp]
		time_vals[i+1] = time.time() - st_time
	filename = 'results/gd_bt_global_'+str(fun_num)+'_abs_fun_num_'+str(abs_fun_num)+'.txt'
...
if algo==4:
	...
	filename = 'results/ibgm_'+str(fun_num)+'_abs_fun_num_'+str(abs_fun_num)+'.txt'
	np.savetxt(filename,np.c_[func_vals, time_vals])
